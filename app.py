"""
The Deven — Studio Booking (Flexible + Meeting Room + Gallery + Rules)
"""
...
RESOURCES = {
    "daylight_studio": {"name": "Daylight Studio", "icon": "📸", "price": "₹1500/hr", "desc": "Photo/Film/Sound"},
    "meeting_room": {"name": "Meeting Room", "icon": "💼", "price": "₹500/hr", "desc": "Meetings/Podcast"}
}
...
def init_db():
    ...
    """)
    # migration: add resource column if old db
    try:
...
def api_book():
    ...
    resource = (data.get("resource") or "daylight_studio").strip()
    if resource not in RESOURCES: resource = "daylight_studio"
